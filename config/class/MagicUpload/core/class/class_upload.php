...
include_once('class/ManipulateImageByImageMagick.php');
...
class Uploadfile
{
	...
	*/
	...
	public function __construct($max_file_size, $root_path, $dest_path, $original_path, $file_ext = NULL, $type = "FILE", $imgMagik) 
	{
		...
		{
			throw new Exception('Il limite massimo di upload, supera il limite impostato sul server.'.self::GetMaxFileSize());
		}
		...
		if($IsMovedFILEOK && $IsSRC_OK) 
		{
			if($this->type_file=='IMG') 
			{
				...
				$arResult = array('error' => UPLFILE_OK, 
													'filename'          => ((preg_match("/avatar/", $upload_dir))? $fileName :'del/'.$fileName), 
													'image'             => $arResultManImage, 
													'filedir'           => $this->dest_path, 
												    'size'              => $file['size'][0],
													'filename_orig'     => $file['name'][0],
                                                    'exif_data'         => $exif_data,
                                                    'average_colour'    => ((preg_match("/br/", $average_colour))? '' : $average_colour),
                                                    'Lat'               => $dLat,
                                                    'Lng'               => $dLong  
                                 );
			} 
            ...
            if(preg_match("/avatar/", $upload_dir)){}else{rename($upload_dir.$fileName, $this->root_path."/".$this->dest_path.'../del/'.$fileName);}
