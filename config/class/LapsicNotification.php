...
class LapsicNotification extends OriginAbstract 
{
	...
	*/
	 ...
	 */	
	public static function GetElementsListRelation($ParametersList)
    {
        ...
        $operation->from(array('H' => PREFIX.'lapsic_notification'),
                        array(  
                                'id_user_source',
                                'status'));         
        if(isset($ParametersList['type']) && ($ParametersList['type'] != 'timer'))
        {
    		$operation->join(array('U' => PREFIX.'lapsic_user'),'U.id = H.id_user_source',
                                    array( 'user' => 'username' ) 
    									);   
        ...
        else
        {
    		...
    		$operation->join(array('U' => PREFIX.'lapsic_user'),'U.id = H.'.((isset($ParametersList['types_timer']) && ($ParametersList['types_timer'] != 'timer'))?'id_user_dest':'id_user_source'));  
        }       
        ...
        (isset($ParametersList['id_element']) && ($ParametersList['id_element'] != ''))? $operation->where("id_record = ".$ParametersList['id_element']) : NULL;
        (isset($ParametersList['own_element']) && ($ParametersList['own_element'] != ''))? $operation->where("id_user_dest = ".$ParametersList['own_element']) : NULL;
        (isset($ParametersList['source']) && ($ParametersList['source'] != ''))? $operation->where("id_user_source = ".$ParametersList['source']) : NULL;        
        ...
        (isset($ParametersList['type']) && ($ParametersList['type'] != ''))? $operation->where("type = '".$ParametersList['type']."'") : NULL;  
        //$operation->where("type = 'timer'");
    ...
    /**
	 * Funzione che permette di recuperare il numero di volte che si è votata una foto
	 * 
	 * @param array $ParametersList Risorsa contenente i filtri da applicare per recuperare i dati richiesti
	 *
	 * @return array $ret Array con i dati ottenuti dalla query su database
	 */	
	public static function GetNumberOfTimed($ParametersList)
    {
	       
        $sql = new Sql($ParametersList['db_adapter']);
        $operation = $sql->select();
        $operation->from(array('H' => PREFIX.'lapsic_notification'),
                        array( 'numb_timed' => 'COUNT(*)'));         
        ...
        (isset($ParametersList['own_element']) && ($ParametersList['own_element'] != ''))? $operation->where("id_user_dest = ".$ParametersList['own_element']) : NULL; 
        (isset($ParametersList['record']) && ($ParametersList['record'] != ''))? $operation->where("id_record = ".$ParametersList['record']) : NULL;
        (isset($ParametersList['source']) && ($ParametersList['source'] != ''))? $operation->where("id_user_source = ".$ParametersList['source']) : NULL;        
        (isset($ParametersList['type']) && ($ParametersList['type'] != ''))? $operation->where("type = '".$ParametersList['type']."'") : NULL;  
        $operation->where('HOUR(TIMEDIFF(data_creation, utc_timestamp())) < 24');
        $operation->order(array('H.data_creation'));
