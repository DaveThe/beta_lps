	...
	*/
    ...
    public static function getContestElements($ParametersList) //$contest)
    {
        ...
        */
         ...
         $res = $ParametersList['db_adapter']->query(
                                                "SELECT p.id, p.description, p.time_left, p.source_path, p.id_owner, u.rank, u.nickname AS user
                                                FROM lapsic_photo AS p
                                                JOIN lapsic_user AS u ON u.id = p.id_owner
                                                WHERE p.description LIKE '%".$ParametersList['contest']."%' AND p.status <> 2 AND p.time_left > UTC_TIMESTAMP GROUP BY p.source_path ORDER BY time_left DESC");
	 ...
	 *
	 */	
	public function ElabHashtag($string, $mode = FALSE) 
    {
        $regex = "/#+([a-zA-Z0-9_]+)/";
        ...
        {
           $string = str_replace($value, '<a target="_parent" class="'.(($mode)?'box-cerca':'').' nounderline fancybox.ajax" style="color:#96cbf3" href="/cerca.php?text='.str_replace('#','',$value).'" data-fancybox-type="ajax">'.$value."</a>", $string);
           //$string = str_replace($value, '<a class="'.(($mode)?'box-cerca':'').' nounderline fancybox.ajax" style="color:#96cbf3" href="/box-cerca.php?text='.str_replace('#','',$value).'" data-fancybox-type="ajax">'.$value."</a>", $string);
        }
