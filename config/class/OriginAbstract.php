...
namespace Lapsic;
/**
 * La class OriginAbstract contiene le funzioni fondamentali per tutte le classi del progetto
 *
 *
 * @version   1.00
 * @since     2015-05-17
...
abstract class OriginAbstract
{
    ...
    {	   
		...
		if ($row = $resData->current()) 
        {
            ...
            $sql_u          = new Sql($db);
            $operation_u    = $sql_u->update(PREFIX.$table);
            $newData        = array('status'  => $status );
            $operation_u->set($newData);
            $operation_u->where('id = '.$id);
            
            /* DEBUG */
    		$debug_sql_u = $sql_u->getSqlStringForSqlObject($operation_u); 
    		Debug::ShowDebug(__FUNCTION__, $debug_sql_u, __CLASS__, basename(__FILE__));
    		/* ***** */
    		//echo $debug_sql_u;
    	    if(self::executeQuery($sql_u,$operation_u,__FUNCTION__, __CLASS__, basename(__FILE__), $debug_sql_u, debug_backtrace(), ID_USER_LOG))
            {
