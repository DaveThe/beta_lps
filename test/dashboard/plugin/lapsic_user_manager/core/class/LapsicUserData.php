...
class LapsicUserData extends OriginAbstract 
{
	...
	*/
	...
	/**
	* Parametro che indica l'id_user dell'utente
	*
	* @var string
	*/
    public $id_user 		    = NULL;
	...
	*/
	...
	public function GetElement($id) 
    {
		...
		if ( isset($id) && ($id != '') ) {
            ...
            $operation->from(  array('S' => PREFIX.'lapsic_user_data'),
                            array(  
                                    'id',
                                    'id_user',
                                    'description',
	 ...
	 */	
	 public function update()
	 {
        ...
        $sql = new Sql($this->db);
		$operation = $sql->update(PREFIX.'lapsic_user_data');
		$newData = array(                            
                            'id_user'      => isset($this->id_user) ? $this->id_user : 'NULL',
    ...
    {		
        ...
        $sql = new Sql($this->db);        
        $operation = $sql->insert(PREFIX.'lapsic_user_data');
        $newData = array(                                                      
                            'id_user'       => isset($this->id_user) ? $this->id_user : 'NULL',
                            'description'   => isset($this->description) ? $this->description : 'NULL',
                            'value'         => isset($this->value) ? $this->value : 'NULL',
    ...
    {	
        return self::DeleteMaster($db, 'lapsic_user_data', 'id', $id);
	}
    ...
    {	   
        return self::ApproveMaster($db, 'lapsic_user_data', $id);
	}
    ...
    {	   
		return self::DisapproveMaster($db, 'lapsic_user_data', $id);
	}
	...
	public static function GetElementsList($ParametersList)
    {
        ...
        $operation = $sql->select();
        $operation->from(array('A' => PREFIX.'lapsic_user_data'),
                        array(  
                                'id',
                                'id_user',
                                'description',
	 ...
	 * @return array $ret Array con i dati ottenuti dalla query su database
	 */	
	public static function GetElementsListByUser($ParametersList)
    {
	       
        $sql = new Sql($ParametersList['db_adapter']);
        $operation = $sql->select();
        $operation->from(array('UD' => PREFIX.'lapsic_user_data'),
                        array(  'id',
                                'description',
                                'value'));
        
		$operation->join(array('GE' => PREFIX.'lapsic_group_extra'),'GE.id = UD.id_group',
										array( 'group_description' => 'description' ) 
									);
		
        (isset($ParametersList['id_user']) && ($ParametersList['id_user'] != ''))? $operation->where("id_user = ".$ParametersList['id_user']) : NULL;
        (isset($ParametersList['own_element']) && ($ParametersList['own_element'] != ''))? $operation->where("created_by = ".$ParametersList['own_element']) : NULL; 
        (isset($ParametersList['text']) && ($ParametersList['text'] != ''))? $operation->where('description LIKE "%'.$ParametersList['text'].'%" OR value LIKE "%'.$ParametersList['text'].'%"') : NULL; 
        //$operation->order(array('data_creation'));
