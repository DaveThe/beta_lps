...
function LapsicPhoto($ids, $ParametersList) 
{
     ...
     */
     ...
     foreach($results->toArray() as $el)
     {
      ...
      $container[] =  
     '<div class="item" style="width: '.$dimensions['width'].'px;height:'.$dimensions['height'].'px;">
                            ...
                            <a class="dettaglio-photo fancybox.ajax" href="/photo.php?id='.$el['id'].'" data-fancybox-type="ajax"><img class="search-hover"  src="../images/zoom-img.png" width="73" height="83" alt="'.$el['id'].'"/></a>
                            <div class="box-date-hour" style="display: block;">
                                <div class="date-item" id="counter_'.$el['id'].'" title="'.$el['time_left'].'" data-countdown="'.$el['time_left'].'">
                                    112d:04h
                                </div>
                                <div class="add-hour" id="'.$el['id'].'">
                                    +24 h
                                </div>
                            </div>
...
function LapsicUser($ids, $ParametersList) 
{
    //echo 'Attivo funzione LapsicUser';
    //echo '-<br>';
...
if(sizeof($components))
{
    ...
    {   
        ...
        elseif($component == 'LapsicUser')
        {
            ...
            if($buggy) print_r($ids);
            if($buggy) echo '-<br>';
            $componentResults[] = LapsicUser($ids, $ParametersList);        
        }
        else
        {
            $undefined++;
