...
{   
	if(sizeof($errors)==0) 
    {
        ...
        if(isset($element->id) && $element->id != '')
        {
            ...
            if (is_array($arr_exif) || is_object($arr_exif))
            {
            ...
            {   
                ...
                {
                    $element_hashtag->id_record =  $element->id;
        ...
        {
            echo ( json_encode( array( 'status' => 'KO', 'zone' => $section ) ) );
        }
        
        if($ret) 
        {
    		echo ( json_encode( array( 'status' => 'OK', 'id_photo' => $element->id ) ) );
    	} 
        else 
        {
    		echo ( json_encode( array( 'status' => 'KO', 'zone' => $section ) ) );
