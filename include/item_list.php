<?php
namespace Lapsic;
function printItems($elements, $my_profile = false)
{
    ...
    ?>
        <div class="item" style="width: <?php echo ($dimensions['width']); ?>px;height: <?php echo ($dimensions['height']); ?>px;">
            <?php  if(PAGE_NAME == 'PROFILE' && (isset($my_profile) && $my_profile) ) { ?>
            <div class="chiudi-iso operations" style="cursor: pointer;" value="elimina" ids="<?php echo( $el['id'] ); ?>" >
            	<a class="" href="#!">X</a>
            </div>
            <?php } ?>
            ...
            <div class="name-profile-photo" style="display: block;">
                <a class="href_nn" style="color: white;" href="profile.php?id=<?php echo ($el['id_owner']); ?>">
                    <?php echo clean_string((strlen($el['user'])>15)?substr($el['user'],0,15).'...':$el['user']); ?>
                </a>
            ...
            <div class="box-dettaglio-profilo fl_none">
              <div class="box-numeri-profilo">
                  <div class="numeri-profilo-left">
                    <p class="nome-profilo-dett"><a class=" nounderline" style="" href="profile.php?id=<?php echo ($el['id']); ?>"><?php echo clean_string( (isset($el['nickname']) && $el['nickname'] != '') ? ((strlen($el['nickname'])>7)?substr($el['nickname'],0,7).'...':$el['nickname']) : ((strlen($el['username'])>7)?substr($el['username'],0,7).'...':$el['username']) ); ?></a></p>
