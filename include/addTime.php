    ...
    {     
        ...
        if( $numb_timed->getTotalItemCount() == 0 )
        {
            ...
            {    	
                ...
                $action     = new LogAction($db, CREATE, __FUNCTION__, __CLASS__, basename(__FILE__), $lapsic_user->id, ID_USER_LOG, "Utente con id: ".$lapsic_user->id." ha donato tempo alla foto id: ".$element->id. " Vecchia data: ".$current.' nuovo '.$new_time);                            
                $ret		= $action->Insert();
...
if($no_login && $already_timed && $own_photo && $ret && $res_ntf && $res_timer) 
{
    ...
    //echo $new_time;
    echo json_encode(array('esito'=>'0','date'=>$new_time, 'time' => strtotime($new_time)));
} 
else 
{
    if(!$no_login)
